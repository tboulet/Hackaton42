...
parser.add_argument("--all_val", type=bool, default=False, help="show all data from val dataset")
...
if args.data_id:
	...
	plt.imshow(X_val[args.data_id, 0], cmap='gray')
	plt.axis('off')
	#plt.set_title(f"Data {args.data_id}", fontsize=7)
	#fig.suptitle(f"Val data {args.data_id}")
	plt.show()
	sys.exit()
...
if args.all_label:
	...
	while x < X_labeled.shape[0]:
		...
		fig.suptitle(f"Labeled data page {x / 100} of {X_labeled.shape[0] / 100}")
		plt.show()
		print(x)
	sys.exit()
...
if args.all_val:
	x = 0
	while x < X_val.shape[0]:
		fig, axs = plt.subplots(10, 10, figsize=(50, 50))
		for i in range(10):
			for j in range(10):
				axs[i, j].imshow(X_val[x, 0], cmap='gray')
				axs[i, j].axis('off')
				axs[i, j].set_title(f"Data {x}", fontsize=7, color="red")
				x = x + 1

		fig.suptitle(f"Val data page {x / 100} of {X_val.shape[0] / 100}")
		plt.show()
		print(x)
	sys.exit()
